 ...
 *
 * @copyright   Copyright (c) 2022 Zhang, Chenbo
...
public:
    ...
    inline map<string, int> SearchPattern(const string &strInput) const
    {
        ...
        int strInputlen = strInput.size();
        for(int i=0;i<strInputlen;i++){
            char chval = CaseConvert(strInput[i]);
            while (true)
            {
                BREAK_ON_NULLPTR(pCurNode);
                auto iterFind = pCurNode->mapChild.find(chval); // search child
